...
trait UserAnswersEntryGenerators extends PageGenerators with ModelGenerators {
  ...
  implicit lazy val arbitraryGoodsQuantitiesInformationUserAnswersEntry: Arbitrary[(GoodsQuantitiesInformationPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[GoodsQuantitiesInformationPage.type]
        value <- arbitrary[String].suchThat(_.nonEmpty).map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryChooseGoodsQuantitiesInformationUserAnswersEntry: Arbitrary[(ChooseGoodsQuantitiesInformationPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[ChooseGoodsQuantitiesInformationPage.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }

  implicit lazy val arbitraryChooseGoodsTypeInformationUserAnswersEntry: Arbitrary[(ChooseGoodsTypeInformationPage.type, JsValue)] =
    Arbitrary {
      for {
        page  <- arbitrary[ChooseGoodsTypeInformationPage.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }
  ...
  implicit lazy val arbitraryConsigneeInformationPageUserAnswersEntry: Arbitrary[(ConsigneeInformationPage.type, JsValue)] =
    Arbitrary {
      for {
        page <- arbitrary[ConsigneeInformationPage.type]
        value <- arbitrary[Boolean].map(Json.toJson(_))
      } yield (page, value)
    }
