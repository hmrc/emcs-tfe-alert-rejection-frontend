  ...
  def onPageLoad(ern: String, arc: String, mode: Mode): Action[AnyContent] =
    authorisedDataRequestWithCachedMovement(ern, arc) { implicit request =>
      if(request.userAnswers.isNorthernIrelandTrader) {
        Ok(view(fillForm(DestinationOfficePage, formProvider()), mode))
      } else {
        Redirect(navigator.nextPage(DestinationOfficePage, mode, request.userAnswers))
      }
    }
    ...
    authorisedDataRequestWithCachedMovementAsync(ern, arc) { implicit request =>
      if (request.userAnswers.isNorthernIrelandTrader) {
        formProvider().bindFromRequest().fold(
          ...
          value =>
            saveAndRedirect(DestinationOfficePage, value, mode)
        )
      } else {
        Future.successful(Redirect(navigator.nextPage(DestinationOfficePage, mode, request.userAnswers)))
      }
  }
