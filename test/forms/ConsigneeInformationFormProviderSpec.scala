...
import fixtures.messages.ConsigneeInformationMessages
import forms.behaviours.StringFieldBehaviours
import org.scalatestplus.play.guice.GuiceOneAppPerSuite
import play.api.data.FormError
import play.api.i18n.{Messages, MessagesApi}

class ConsigneeInformationFormProviderSpec extends StringFieldBehaviours with GuiceOneAppPerSuite {

  val requiredKey = "consigneeInformation.error.required"
  val lengthKey = "consigneeInformation.error.length"
  val xssKey = "consigneeInformation.error.xss"
  val characterKey = "consigneeInformation.error.character"
  val maxLength = 350
  val fieldName = "value"

  ".value" - {

    val form = new ConsigneeInformationFormProvider().apply()
