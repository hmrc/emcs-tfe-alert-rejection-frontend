...
class ChooseGoodsTypeInformationControllerSpec extends SpecBase with MockUserAnswersService {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new ChooseGoodsTypeInformationFormProvider()
  val form = formProvider()

  lazy val chooseGoodsTypeInformationRoute = routes.ChooseGoodsTypeInformationController.onPageLoad(testErn, testArc, NormalMode).url

  "ChooseGoodsTypeInformation Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, chooseGoodsTypeInformationRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ChooseGoodsTypeInformationView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(ChooseGoodsTypeInformationPage, true)
