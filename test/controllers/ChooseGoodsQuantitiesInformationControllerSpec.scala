...
class ChooseGoodsQuantitiesInformationControllerSpec extends SpecBase with MockUserAnswersService {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new ChooseGoodsQuantitiesInformationFormProvider()
  val form = formProvider()

  lazy val chooseGoodsQuantitiesInformationRoute = routes.ChooseGoodsQuantitiesInformationController.onPageLoad(testErn, testArc, NormalMode).url

  "ChooseGoodsQuantitiesInformation Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, chooseGoodsQuantitiesInformationRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[ChooseGoodsQuantitiesInformationView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(ChooseGoodsQuantitiesInformationPage, true)
