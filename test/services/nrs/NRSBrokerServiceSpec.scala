...
import models.nrs.AlertRejectNRSSubmission
  ...
  ".submitPayload" - {
    ...
    "should return a Left" - {
      ...
      "when retrieving the identity data throws an exception" in {
        ...
        await(service.submitPayload(submitAlertOrRejectionModel, testErn)) mustBe Left(IdentityDataException("UnsupportedAuthProvider"))
      }
      ...
      "when the broker connector returns a Left" in {
        ...
        MockNRSBrokerConnector.submitPayload(createNRSPayload(AlertRejectNRSSubmission(testErn, submitAlertOrRejectionModel)), testErn).returns(Future.successful(Left(UnexpectedDownstreamResponseError)))

        await(service.submitPayload(submitAlertOrRejectionModel, testErn)) mustBe Left(UnexpectedDownstreamResponseError)
      }

      "when the broker connector returns an unhandled future exception" in {

        MockAuthConnector.authorise(EmptyPredicate, retrievals).returns(Future.successful(predicateRetrieval))
        MockNRSBrokerConnector.submitPayload(createNRSPayload(AlertRejectNRSSubmission(testErn, submitAlertOrRejectionModel)), testErn).returns(Future.failed(new Exception("Game over!")))
    ...
    "should return a Right" - {
      ...
      "when the identity data is retrieved successfully and the payload is submitted to the broker correctly" in {
        ...
        MockAuthConnector.authorise(EmptyPredicate, retrievals).returns(Future.successful(predicateRetrieval))
        MockNRSBrokerConnector.submitPayload(createNRSPayload(AlertRejectNRSSubmission(testErn, submitAlertOrRejectionModel)), testErn).returns(Future.successful(Right(nrsBrokerResponseModel)))

        await(service.submitPayload(submitAlertOrRejectionModel, testErn)) mustBe Right(nrsBrokerResponseModel)
      }
    }
  }
}
